...
try{
  //make sqlconnection
  $verbinding = mysql_connect(MYSQL_SERVER, MYSQL_GEBRUIKERSNAAM, MYSQL_PASWOORD) or
...
try{
	...
	//make curl rquest
	$url = "http://api.irail.be/connections/?to=".$arrival."&from=".$departure . "&lang=NL";
	//read xml file
...
//$GLOBALS['firephp']->log($connections,"xml output");
	...
	for ($selectedInterval; $selectedInterval <= $maxInterval; $selectedInterval++){
	  ...
	  $formattedDateTime = formatDate("04","07",$year) . formatTime($hour,$minutes,$selectedInterval);
		...
		foreach($connections->connection as $item){
			...
			if($query == $formattedDateTime){
				...
				$con->effectiveDepartureTime= formatTime(20, 29 , $departureDelay);//calculate time
				$con->effectiveArrivalTime= formatTime(20, 45 , $arrivalDelay);//calculate time
...
//cut the 4 last numbers
	$vehicleAccuratePosition = substr($vehicle, (strlen($vehicle) -4) , 4 );//length  -4
